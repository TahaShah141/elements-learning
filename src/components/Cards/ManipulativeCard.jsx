...
export const ManipulativeCard = ({name, src, color, innerColor, text, buyLink, index}) => {
  ...
  return (
    <SlidingDiv direction={"bottom"} delay={index*0.25} className="relative group/manipulative flex gap-4 items-center rounded-3xl rounded-bl-[96px] text-2xl rounded-tl-none" style={{backgroundColor: color}}>
      <Link to={buyLink} className="relative size-16 xs:size-24 sm:size-32 md:size-40 lg:size-48 rounded-full rounded-tl-none overflow-hidden p-4 sm:p-8 md:p-10 lg:p-12" style={{backgroundColor: innerColor}}>
      ...
      <Link to={buyLink} target="_blank" className="absolute hidden min-[700px]:block right-0 bottom-0" >
